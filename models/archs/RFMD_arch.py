...
from .src.modules import ConvMLPStage
...
class RFMD(nn.Module):
    def __init__(self,cfg):
        super(RFMD, self).__init__()
        scale = cfg["scale"]
    ...
    def forward(self,x,scale):
        ...
        p2= self.N1_IMG(x, vgg_feature, scale)
...
class N2(nn.Module):
    def __init__(self, bilinear=True, out_chans=1):
        ...
        self.down2 = SKDown(3, 1, False, 16, 32, 64,32,1024)
        self.down3 = SKDown(3, 1, False, 16, 64, 64,64,1024)
        self.up1 = SKUp(3, 1, False, 16, 128, 32,128,1024, bilinear=False)
        self.up2 = SKUp(3, 1, False, 16, 64, 32, 64,1024, bilinear=False)
        self.up4 = nn.Conv2d(kernel_size=3, padding=1, in_channels=32, out_channels = out_chans)


    def forward(self, x,vgg_feature, scale=3):
        x1 =  self.Conv1(x)
        x1 = self.Conv(x1)
        x2 = self.down2(x1,vgg_feature)
        x3 = self.down3(x2,vgg_feature)
...
class SelectiveConv(nn.Module):
    def __init__(self,kernel_size, padding, bias, reduction, in_channels, out_channels,embedding_dim, dim_feedforward, first=False):
        ...
        self.ConvMLPStage =ConvMLPStage(embedding_dim, dim_feedforward=dim_feedforward, stochastic_depth_rate=0.1)
        self.ConvMLPStage1 = ConvMLPStage(embedding_dim, dim_feedforward=dim_feedforward, stochastic_depth_rate=0.1)
    ...
    def forward(self, x,vgg_feature):
        ...
        # redundant feature reprocessing
...
class SKDown(nn.Module):
    def __init__(self, kernel_size, padding, bias, reduction, in_channels, out_channels,embedding_dim, dim_feedforward, first=False):
        ...
        self.conv = SelectiveConv(kernel_size, padding, bias, reduction, in_channels, out_channels,embedding_dim,dim_feedforward, first=first)


    def forward(self, x,vgg_feature):
        return self.conv(self.maxpool(x),vgg_feature)
